...
(define (execute-extract-deepest recipe append-to)
  (cond ((base-component? recipe)
        ...
        ((all-base-components? recipe)
            ...
            `( ,(find-in-dict-list 'name: (cadar recipe)) (from-step: ,(- (length append-to) 1))))
        (else
            (execute-extract-deepest (map (lambda (x) (execute-extract-deepest x append-to)) recipe) append-to)
            `( ,(find-in-dict-list 'name: (cadar recipe)) (from-step: ,(- (length append-to) 1))))))
